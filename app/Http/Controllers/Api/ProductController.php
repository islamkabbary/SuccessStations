...
{
    public function productsInBrand(Request $request)
    {
        try {
            $products = Product::where('brand_id', $request->brand_id)->whereHas('company', function ($query) use ($request) {
                $query->where('id', $request->company_id);
            })->get();
            return response()->json(['status' => 1, 'code' => 200, 'message' => trans(''), 'data' => ProductResource::collection($products)], Response::HTTP_OK);
        } catch (\Throwable $th) {
